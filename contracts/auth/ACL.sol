...
contract Auth {
    ...
    */
    function verifyJWT(bytes32 challenge, bytes signedJWTHash) private {
        // TBD verify the signature of consumer
        return true;
    }

    function verifyAccessTokenDelivery(bytes challenge, bytes signedJWTHash) public onlyProvider
    isAccessCommitted(challenge) {
        ...
        if (accessHandler[challenge].consent.expire < now) {
            ...
            if (verifyJWT(challenge, jwtHash)) {
